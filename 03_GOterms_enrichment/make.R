...
# Author: Thomas Wolfe / Aglaia Szukala
...
# Load data
geneID2GO <- readMappings(file = "/path/to/geneID2GO.txt")
GO2geneID <- inverseList(geneID2GO)
geneNames <- names(geneID2GO)
deGenes <- read.table("/path/to/DEGs_list.txt")
resEdgeRtopTags <- read.table("/path/to/DEGs.txt")
dat.filtered <- read.table("/path/to/dataFileredCounts.txt", header = T)

# Perform GOplot analysis
plan <- drake_plan(
...
make(plan)
# Data is saved in the .drake folder as .rds files. 
# Only change the variables needed when the script is modified.

# Retrieve the circ variable from the .drake folder.
circ <- readd(circ)
write.table(circ, file ="file_out.txt", sep = "\t", row.names = F, quote = F)
...
GOBubble(reduced_circ, labels = 2, display = 'multiple', table.legend = T)

# Plot selected GO terms

plot_GO(data= "/path/to/GOterms.txt","pair1")
